...
export default new Vuex.Store({
    state: {
        api_endpoint:
            "https://www.randomtext.me/api/gibberish/p-1/5-10?_=1608182114179",
        type_message: {
            count: -1,
    ...
    mutations: {
        ...
        load_games: function(state) {
            let games = JSON.parse(localStorage.getItem("games"));
            let i;
            for(i=0;i<games.length;i++){
                state.games.push(games[i])
            }
        }
    },
    actions: {
        load: ({ commit }) => {
            commit("load_games");
            console.log("preloading saved game stats");
        },
        ...
        stop: ({ commit, getters }) => {
            if (getters.get_type_state.flag) {
                ...
                let re =
                    (getters.get_games[getters.get_type_state.current_id]
                        .gameTimer.end -
                        getters.get_games[getters.get_type_state.current_id]
                            .gameTimer.start) /
                    1000;
                ...
                commit("set_wpm", {
                    wpm: wpm,
                    arg: getters.get_type_state.current_id,
                });
                console.log(
                    getters.get_games[getters.get_type_state.current_id]
                );
